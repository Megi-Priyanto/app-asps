            ...
            <div class="nav-notif-dropdown">
                ...
                <div class="nav-notif-menu" id="navNotifMenu">
                    ...
                    <div style="max-height: 300px; overflow-y: auto;">
                        @if(isset($notifKomentarList) && $notifKomentarList->count() > 0)
                            @foreach($notifKomentarList as $notif)
                                <a href="{{ route('pegawai.laporan.show', $notif->laporan_id) }}" class="nav-notif-item">
                    ...
                    <div class="nav-notif-footer">
                        <a href="{{ route('pegawai.laporan.index') }}">Buka Semua Laporan</a>
                    </div>
                </div>
            </div>
            ...
            <div class="nav-user-dropdown">
                <button class="nav-user-trigger" id="navUserTrigger">
                    <div class="nav-user-avatar">
                        @if(auth('pegawai')->user()->foto)
                            <img src="{{ asset('storage/' . auth('pegawai')->user()->foto) }}" alt="Avatar" style="width:100%; height:100%; object-fit:cover; border-radius:9px;">
                        @else
...
<div class="nav-mobile-menu" id="navMobileMenu">
    <div style="padding: 10px 14px 8px; display:flex; align-items:center; gap:10px;">
        <div style="width:36px;height:36px;border-radius:9px;background:linear-gradient(135deg,#2563EB,#60A5FA);display:flex;align-items:center;justify-content:center;color:white;font-weight:700;font-size:13px;overflow:hidden;">
            @if(auth('pegawai')->user()->foto)
                <img src="{{ asset('storage/' . auth('pegawai')->user()->foto) }}" alt="Avatar" style="width:100%; height:100%; object-fit:cover;">
            @else
                {{ strtoupper(substr(auth('pegawai')->user()->nama, 0, 2)) }}
            @endif
        </div>
    ...
    <a href="{{ route('pegawai.peminjaman-barang.index') }}" class="nav-mobile-item {{ request()->routeIs('pegawai.peminjaman-barang.*') ? 'primary':'' }}">
...
<nav class="apss-bottom-nav" id="apssBottomNav">
    <div class="bottom-nav-inner">
        <a href="{{ route('pegawai.dashboard') }}" class="bottom-nav-item {{ request()->routeIs('pegawai.dashboard') ? 'active' : '' }}">
            <i class="bi bi-grid-1x2-fill"></i>
            <span>Dashboard</span>
        </a>
        <a href="{{ route('pegawai.laporan.index') }}" class="bottom-nav-item {{ request()->routeIs('pegawai.laporan.*') ? 'active' : '' }}">
            <i class="bi bi-file-earmark-text-fill"></i>
            <span>Laporan</span>
        </a>
        <a href="{{ route('pegawai.peminjaman-barang.index') }}" class="bottom-nav-item {{ request()->routeIs('pegawai.peminjaman-barang.*') ? 'active' : '' }}">
            <i class="bi bi-box-seam-fill"></i>
            <span>Peminjaman</span>
        </a>
        <a href="{{ route('pegawai.tanggapan.index') }}" class="bottom-nav-item {{ request()->routeIs('pegawai.tanggapan.*') ? 'active' : '' }}">
            <i class="bi bi-chat-left-text-fill"></i>
            <span>Tanggapan</span>
        </a>
        <a href="{{ route('pegawai.akun') }}" class="bottom-nav-item {{ request()->routeIs('pegawai.akun*') ? 'active' : '' }}">
