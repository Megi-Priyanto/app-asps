...
<div class="card">
    <div class="card-header d-flex flex-wrap align-items-center gap-2">
        ...
        <div class="ms-auto d-flex gap-2 flex-wrap">
            <form method="GET" action="{{ route('superadmin.pengguna.pegawai.index') }}" class="d-flex gap-2">
                <input type="text" name="search" class="form-control form-control-sm"
                    placeholder="Cari Username / Nama / Jabatan..." value="{{ request('search') }}" style="width:240px;">
                @if(request('search'))
                    <a href="{{ route('superadmin.pengguna.pegawai.index') }}" class="btn btn-sm btn-secondary">Reset</a>
                @endif
            </form>

            {{-- Download Template --}}
            <a href="{{ route('superadmin.pengguna.pegawai.template') }}" class="btn btn-success d-inline-flex align-items-center justify-content-center" style="line-height: 1; height: 36px; font-size: 13px; font-weight: 600; padding: 0 16px; border-radius: 8px;">
            ...
            <div class="dropdown">
                ...
                <ul class="dropdown-menu dropdown-menu-end shadow-sm" style="font-size:13px; border-radius:10px; border:1px solid #E2E8F0;">
                    <li><a class="dropdown-item py-2" href="{{ route('superadmin.pengguna.pegawai.export.excel') }}"><i class="bi bi-file-earmark-excel text-success me-2"></i>Export Excel (.xlsx)</a></li>
                    <li><a class="dropdown-item py-2" href="{{ route('superadmin.pengguna.pegawai.export.pdf') }}"><i class="bi bi-file-earmark-pdf text-danger me-2"></i>Export PDF (.pdf)</a></li>
                </ul>
            </div>

            <a href="{{ route('superadmin.pengguna.pegawai.create') }}" class="btn btn-primary d-inline-flex align-items-center justify-content-center" style="line-height: 1; height: 36px; font-size: 13px; font-weight: 600; padding: 0 16px; border-radius: 8px;">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="d-flex justify-content-center gap-1">
                                    <a href="{{ route('superadmin.pengguna.pegawai.edit', $item->id) }}" class="btn btn-sm btn-secondary">
                                        Edit
                                    </a>
                                    <form action="{{ route('superadmin.pengguna.pegawai.destroy', $item->id) }}" method="POST" class="m-0">
                                        @csrf @method('DELETE')
        ...
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="p-3 mb-3 rounded-3" style="background:#FFFBEB;border:1px solid #FDE68A;">
                    ...
                    <a href="{{ route('superadmin.pengguna.pegawai.template') }}" class="btn btn-sm btn-success">
                        <i class="bi bi-file-earmark-excel me-1"></i>Unduh Format Excel
                    </a>
                </div>

                <form action="{{ route('superadmin.pengguna.pegawai.import') }}" method="POST" enctype="multipart/form-data" id="formImportPegawai">
                    @csrf
                    <div class="mb-3">
