...
<div class="card">
    <div class="card-header d-flex flex-wrap align-items-center gap-2">
        ...
        <div class="ms-auto d-flex gap-2 flex-wrap">
            <form method="GET" action="{{ route('superadmin.laporan.index') }}" class="d-flex gap-2">
                {{-- Filter Peran --}}
                ...
                @if(request('status') || request('role'))
                    <a href="{{ route('superadmin.laporan.index') }}" class="btn btn-sm btn-secondary">Reset</a>
                @endif
            </form>
    ...
    <div class="card-body p-0">
        <div class="table-responsive">
            @include('superadmin.laporan.list')
        </div>
    </div>
...
<script>
    ...
    function doCetak() {
        const jenis = document.querySelector('input[name="jenis_laporan"]:checked').value;
        const base  = '{{ route('superadmin.laporan.cetak') }}';
        let url = '';
